...
- **Показывает, что играет.** Любой плеер, который сообщает о себе Windows: обложка и
  эквалайзер в капсуле, карточка с кнопками в раскрытом островке.
...
| Живая капсула | идёт таймер или прогресс плагина; играет музыка — обложка и эквалайзер |
...
| `ct ` | последние загрузки ClipTide |
...
- **ClipTide.** Если на машине стоит [ClipTide](https://github.com/Rayness/YouTube-Downloader),
  готовые загрузки появляются пиком с превью, щелчок открывает папку. Правок в ClipTide
  не нужно — островок следит за его файлом уведомлений.
...
| Уведомления | пик, метка на полоске или тихо; сколько держится пик; звук; живые активности, музыка в капсуле, карточка «Сейчас играет» |
...
| Интеграции | вход в Kawaki, уведомления и поиск Kawaki, уведомления ClipTide, примеры для своих скриптов |
...
| `Media/` | «Сейчас играет» через системные медиасеансы Windows |
